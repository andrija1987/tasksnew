...
$milestones_edit = NULL; // Initialize page object first

class cmilestones_edit extends cmilestones {
	...
	// Page object name
	var $PageObjName = 'milestones_edit';
	...
	function Page_Main() {
		...
		// Check if valid key
		if ($this->milestoneid->CurrentValue == "")
			$this->Page_Terminate("milestoneslist.php"); // Invalid key, return to list
	...
	function LoadFormValues() {
		...
		// Load from form
		global $objForm;
		if (!$this->milestoneid->FldIsDetailKey)
			$this->milestoneid->setFormValue($objForm->GetValue("x_milestoneid"));
	...
	function RestoreFormValues() {
		global $objForm;
		$this->LoadRow();
		$this->milestoneid->CurrentValue = $this->milestoneid->FormValue;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// milestoneid
			$this->milestoneid->EditAttrs["class"] = "form-control";
			$this->milestoneid->EditCustomAttributes = "";
			$this->milestoneid->EditValue = $this->milestoneid->CurrentValue;
			$this->milestoneid->ViewCustomAttributes = "";
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// milestonename
			$this->milestonename->SetDbValueDef($rsnew, $this->milestonename->CurrentValue, "", $this->milestonename->ReadOnly);

			// startdate
			$this->startdate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->startdate->CurrentValue, 5), ew_CurrentDate(), $this->startdate->ReadOnly);

			// enddate
			$this->enddate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->enddate->CurrentValue, 5), ew_CurrentDate(), $this->enddate->ReadOnly);
...
// Create page object
if (!isset($milestones_edit)) $milestones_edit = new cmilestones_edit();

// Page init
$milestones_edit->Page_Init();

// Page main
$milestones_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$milestones_edit->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var milestones_edit = new ew_Page("milestones_edit");
milestones_edit.PageID = "edit"; // Page ID
var EW_PAGE_ID = milestones_edit.PageID; // For backward compatibility

// Form object
var fmilestonesedit = new ew_Form("fmilestonesedit");

// Validate form
fmilestonesedit.Validate = function() {
...
fmilestonesedit.Form_CustomValidate = 
...
fmilestonesedit.ValidateRequired = true;
<?php } else { ?>
fmilestonesedit.ValidateRequired = false; 
<?php } ?>
...
<?php $milestones_edit->ShowPageHeader(); ?>
<?php
$milestones_edit->ShowMessage();
?>
<form name="fmilestonesedit" id="fmilestonesedit" class="form-horizontal ewForm ewEditForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($milestones_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $milestones_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="milestones">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<div>
<?php if ($milestones->milestoneid->Visible) { // milestoneid ?>
	<div id="r_milestoneid" class="form-group">
		<label id="elh_milestones_milestoneid" class="col-sm-2 control-label ewLabel"><?php echo $milestones->milestoneid->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $milestones->milestoneid->CellAttributes() ?>>
<span id="el_milestones_milestoneid">
<span<?php echo $milestones->milestoneid->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $milestones->milestoneid->EditValue ?></p></span>
</span>
<input type="hidden" data-field="x_milestoneid" name="x_milestoneid" id="x_milestoneid" value="<?php echo ew_HtmlEncode($milestones->milestoneid->CurrentValue) ?>">
<?php echo $milestones->milestoneid->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<script type="text/javascript">
ew_CreateCalendar("fmilestonesedit", "x_startdate", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
ew_CreateCalendar("fmilestonesedit", "x_enddate", "%Y/%m/%d");
</script>
<?php } ?>
</span>
...
<script type="text/javascript">
fmilestonesedit.Init();
</script>
<?php
$milestones_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$milestones_edit->Page_Terminate();
?>
