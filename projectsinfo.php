...
class cprojects extends cTable {
	...
	function __construct() {
		...
		// projectid
		$this->projectid = new cField('projects', 'projects', 'x_projectid', 'projectid', '`projectid`', '`projectid`', 3, -1, FALSE, '`projectid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->projectid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['projectid'] = &$this->projectid;

		// projectname
		$this->projectname = new cField('projects', 'projects', 'x_projectname', 'projectname', '`projectname`', '`projectname`', 200, -1, FALSE, '`projectname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['projectname'] = &$this->projectname;

		// projectstartdate
		$this->projectstartdate = new cField('projects', 'projects', 'x_projectstartdate', 'projectstartdate', '`projectstartdate`', 'DATE_FORMAT(`projectstartdate`, \'%Y/%m/%d\')', 133, 5, FALSE, '`projectstartdate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->projectstartdate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['projectstartdate'] = &$this->projectstartdate;

		// projectenddate
		$this->projectenddate = new cField('projects', 'projects', 'x_projectenddate', 'projectenddate', '`projectenddate`', 'DATE_FORMAT(`projectenddate`, \'%Y/%m/%d\')', 133, 5, FALSE, '`projectenddate`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->projectenddate->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['projectenddate'] = &$this->projectenddate;

		// budget
		$this->budget = new cField('projects', 'projects', 'x_budget', 'budget', '`budget`', '`budget`', 200, -1, FALSE, '`budget`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('projectid', $rs))
				ew_AddFilter($where, ew_QuotedName('projectid') . '=' . ew_QuotedValue($rs['projectid'], $this->projectid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->projectid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@projectid@", ew_AdjustSql($this->projectid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->projectid->CurrentValue)) {
			$sUrl .= "projectid=" . urlencode($this->projectid->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["projectid"]; // projectid
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->projectid->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->projectname->Exportable) $Doc->ExportCaption($this->projectname);
					if ($this->projectstartdate->Exportable) $Doc->ExportCaption($this->projectstartdate);
					if ($this->projectenddate->Exportable) $Doc->ExportCaption($this->projectenddate);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->projectname->Exportable) $Doc->ExportField($this->projectname);
						if ($this->projectstartdate->Exportable) $Doc->ExportField($this->projectstartdate);
						if ($this->projectenddate->Exportable) $Doc->ExportField($this->projectenddate);
