...
class ctasks extends cTable {
	...
	function __construct() {
		...
		// taskid
		$this->taskid = new cField('tasks', 'tasks', 'x_taskid', 'taskid', '`taskid`', '`taskid`', 3, -1, FALSE, '`taskid`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->taskid->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['taskid'] = &$this->taskid;

		// taskname
		$this->taskname = new cField('tasks', 'tasks', 'x_taskname', 'taskname', '`taskname`', '`taskname`', 200, -1, FALSE, '`taskname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['taskname'] = &$this->taskname;

		// comment
		$this->comment = new cField('tasks', 'tasks', 'x_comment', 'comment', '`comment`', '`comment`', 200, -1, FALSE, '`comment`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['comment'] = &$this->comment;

		// status
		$this->status = new cField('tasks', 'tasks', 'x_status', 'status', '`status`', '`status`', 200, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('taskid', $rs))
				ew_AddFilter($where, ew_QuotedName('taskid') . '=' . ew_QuotedValue($rs['taskid'], $this->taskid->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->taskid->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@taskid@", ew_AdjustSql($this->taskid->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("tasksview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("tasksview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "tasksadd.php?" . $this->UrlParm($parm);
		else
			return "tasksadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("tasksedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("tasksadd.php", $this->UrlParm($parm));
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->taskid->CurrentValue)) {
			$sUrl .= "taskid=" . urlencode($this->taskid->CurrentValue);
		} else {
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->taskid->Exportable) $Doc->ExportCaption($this->taskid);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->taskid->Exportable) $Doc->ExportField($this->taskid);
